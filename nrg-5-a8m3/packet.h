...
static int packet_prepend_data(packet_t *p, uint8_t *bytes, int size) {
    assert(p);
    assert(bytes);
    if ((p->data - p->buf) < size) {
        LOG_DEBUG("packet_prepend_data fail, don't have enough space\n");
        return -1;
    }
    p->data -= size;
    memcpy(p->data, bytes, size);
    p->byte_len += size;
    return 0;
}
